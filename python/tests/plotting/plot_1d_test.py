...
import numpy as np
import scipp as sc
from ..factory import make_variable, make_dense_data_array, make_dense_dataset
from .validation import check_axis_range
...
def test_plot_variable_1d():
    ...
    # x-axis
    check_axis_range(axrange=p.ax.get_xlim(),
                     var=sc.arange(var.dims[0], float(len(var))))
    assert p.ax.get_xlabel() == 'xx'

    # y-axis
    assert p.ax.get_ylabel() == '[counts]'
    assert np.allclose(p.ax.lines[0].get_ydata(), var.values)
    check_axis_range(axrange=p.ax.get_ylim(), var=var)


def test_plot_dict_of_variables_1d():
    v1 = make_variable(ndim=1)
    v2 = 5.0 * v1
    data = {'v1': v1, 'v2': v2}
    p = sc.plot(data)

    # x-axis
    check_axis_range(axrange=p.ax.get_xlim(), var=sc.arange(v1.dims[0], float(len(v1))))
    assert p.ax.get_xlabel() == 'xx'
