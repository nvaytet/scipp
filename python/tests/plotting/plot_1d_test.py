...
@pytest.mark.parametrize("resampling_mode", [None, 'sum', 'mean'])
# @pytest.mark.parametrize("norm", [None, 'linear', 'log'])
# @pytest.mark.parametrize("scale", [None, {'x': 'linear'}, {'x': 'log'}])
def test_plot_1d(with_variance, binedges, labels, attrs, resampling_mode
                 # , norm, scale
                 ):
    ...
    if labels:
        kwargs["labels"] = {"xx": "lab"}
    if attrs:
        kwargs["labels"] = {"xx": "attr"}
    # if norm == "log":
